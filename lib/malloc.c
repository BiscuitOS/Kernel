...
static inline void init_bucket_desc(void)
{
	...
	if (!bdesc)
		panic("Out of memory in inin_bucket_desc()");
...
/*
 * Here is the free routine. If you know the size of object that you
 * are freeing, then free_s() will use that information to speed up the
 * search for the bucket descriptor.
 *
 * We will #define a macro so that "kfree(x)" is becomes "free_s(x, 0)"
 */
