 ...
 *
 * This also takes care to the few special cases due to '..' - traversal
 ...
 * get_dir
 *
 * getdir traverse the pathname until it hits the topmost directory.
 ...
 * dir_namei()
 *
 * dir_name() returns the inode of the directory of the specified name,
...
int sys_d_mknod(const char *filename, int mode, int dev)
{
    const char *basename;
    int namelen;
...
int debug_syscall_mknod0(void)
{
    /* create a character file */
    d_mknod("/dev/biscuit-tty", S_IFCHR | S_IWUSR | S_IRUSR | S_IRGRP | 
                       S_IROTH, 3);

    /* create a directory */
    d_mknod("/usr/biscuitos", S_IFDIR | S_IWUSR | S_IRUSR | S_IRGRP | 
                       S_IROTH | S_ISVTX, 3);
