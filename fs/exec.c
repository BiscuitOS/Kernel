 ...
 * whether the string and the stirng array are from user or kernel segments:
 *
 * from_kmem    argv *        argv **
            ...
            if (--offset < 0) {
                ...
                if (!(pag = (char *)page[p / PAGE_SIZE]) &&
                    !(pag = (char *)(page[p / PAGE_SIZE] =
                      (unsigned long)(unsigned long *)get_free_page())))
                    return 0;
                if (from_kmem == 2)
                    set_fs(new_fs);
            }
...
restart_interp:
    ...
    if ((bh->b_data[0] == '#') && (bh->b_data[1] == '!') && (!sh_bang)) {
         ...
         * user envirnoment and arguments are stored.
         */
