...
void msdos_statfs(struct super_block *sb,struct statfs *buf)
{
	...
	put_fs_long(sb->s_magic, (unsigned long *)&buf->f_type);
	put_fs_long(MSDOS_SB(sb)->cluster_size*SECTOR_SIZE, (unsigned long *)&buf->f_bsize);
	put_fs_long(MSDOS_SB(sb)->clusters, (unsigned long *)&buf->f_blocks);
	lock_fat(sb);
	...
	unlock_fat(sb);
	put_fs_long(free, (unsigned long *)&buf->f_bfree);
	put_fs_long(free, (unsigned long *)&buf->f_bavail);
