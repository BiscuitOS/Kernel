			     ...
			     struct inode * interpreter_inode)
{
	...
	old_fs = get_fs();
	set_fs(get_ds());
	read_exec(interpreter_inode, interp_elf_ex->e_phoff, (char *) elf_phdata,
...
int load_elf_binary(struct linux_binprm * bprm, struct pt_regs * regs)
{
	...
	do_mmap(NULL, 0, 4096, PROT_READ | PROT_EXEC,
			MAP_FIXED | MAP_PRIVATE, 0);
...
int load_elf_library(int fd){
	...
	old_fs = get_fs();
	set_fs(get_ds());
	read_exec(inode, elf_ex.e_phoff, (char *) elf_phdata,
